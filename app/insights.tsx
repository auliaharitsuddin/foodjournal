  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }} edges={['top']}>
      <View style={{ flexDirection: 'row', alignItems: 'center', padding: spacing.lg, gap: spacing.sm }}>
        ...
        <Text style={[type.title2, { color: colors.label }]}>{t.insightsPageTitle}</Text>
      </View>

      <ScrollView contentContainerStyle={{ padding: spacing.lg, paddingTop: 0, paddingBottom: 140 }} showsVerticalScrollIndicator={false}>
        <Text style={[type.footnote, { color: colors.labelSecondary, marginBottom: spacing.md }]}>{t.aiRecommendationLabel}</Text>
        <Card style={{ marginBottom: spacing.xl }}>
          {state.aiRecommendation ? (
            <>
              ...
              <Text style={[type.caption1, { color: colors.labelTertiary, marginBottom: spacing.md }]}>
                {t.updatedAgo(timeAgo(state.aiRecommendation.generatedAt, t))}
              </Text>
            </>
          ) : (
          ...
          <Button
            title={loading ? t.loadingLabel : state.aiRecommendation ? t.updateRecommendation : t.getRecommendation}
            onPress={fetchRecommendation}
            disabled={loading}
          />
        </Card>

        <Text style={[type.footnote, { color: colors.labelSecondary, marginBottom: spacing.md }]}>{t.patternsFound}</Text>
        {!hasEnoughData ? (
